...
void crete_insert_instr_stack_depth_bounds(uint32_t low, uint32_t high)
{
    __asm__ __volatile__(
...
static int __init crete_vm_cmmon_init(void)
{
    printk(KERN_INFO "CRETE-VM-COMM Loaded!\n");
    return 0;    // Non-zero return means that the module couldn't be loaded.
}

static void __exit crete_vm_cmmon_exit(void)
{
}
...
// for Debuging
void crete_test(int a)
{
    printk(KERN_INFO "crete_test() is called: a = %d\n", a);
}
EXPORT_SYMBOL(crete_test);

module_init(crete_vm_cmmon_init);
module_exit(crete_vm_cmmon_exit);
