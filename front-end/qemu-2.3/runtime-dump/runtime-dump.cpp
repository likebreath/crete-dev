...
// Information for replaying Virtual Device operations,
// mainly entry function calls and DMA
class VirtualDeviceOpsInfo
{
private:
    boost::unordered_set<uint64_t> m_vd_entry_func;
    creteVDTable_ty m_vd_table;

public:
    VirtualDeviceOpsInfo();

    // Information for replaying VD entry function operations
    void add_entry_func_info(uint64_t vaddr, uint64_t physaddr,
            uint32_t size, uint64_t value, int is_write, uint64_t accessor)
    {
        if (m_vd_entry_func.find(accessor) !=
                m_vd_entry_func.end())
        {
    ...
    creteVDTable_ty get()
    {
        return m_vd_table; // add marker to "normal" execution trace
    }
};

VirtualDeviceOpsInfo::VirtualDeviceOpsInfo()
{
    for(int i = 0; crete_vd_trace_accessor[i] != 0; ++i)
    {
        ...
        m_vd_entry_func.insert(crete_vd_trace_accessor[i]);
    }
}

static VirtualDeviceOpsInfo vd_ops_info;
  ...
  m_new_tb(false)
{
    ...
    m_flag_HWState_pre_interest= false;
...
RuntimeEnv::~RuntimeEnv()
{
    ...
    assert(m_cpuState_post_insterest);
    delete [] (uint8_t *)m_cpuState_post_insterest;
...
void RuntimeEnv::dump_tloCtx(void *cpuState, TranslationBlock *tb, uint64_t crete_interrupted_pc)
{
    ...
    assert(m_flag_HWState_pre_interest);
    memcpy(cpuState, m_cpuState_pre_interest, sizeof(CPUArchState));
...
void RuntimeEnv::addInitialHardwareState()
{
    assert(m_flag_HWState_pre_interest);
...
{
    assert(m_flag_HWState_post_interest == true);
    assert(m_flag_HWState_pre_interest== true);

    // CPUState
    {
        const CPUArchState *post_interest = (const CPUArchState *) m_cpuState_post_insterest;
...
void RuntimeEnv::addCurrentMemoSyncTableEntry(uint64_t addr, uint32_t size, uint64_t value)
{
    ...
    for(uint64_t i = 0; i < size ; ++i){
        ...
        addCurrentMemoSyncTableEntryByte(addr + i, byte_value);
    }
}

void RuntimeEnv::addCurrentMemoSyncTableEntryByte(uint64_t addr, uint8_t byte_value)
{
    m_currentMemoSyncTable.insert(make_pair(addr, byte_value));
}
...
{
    assert(m_flag_HWState_post_interest == false &&
            "m_flag_HWState_post_interest should always be invalided in addHardwareStateSyncTable().\n");
...
void RuntimeEnv::writeInitialHardwareState()
{
    // CPU State
    {
...
void RuntimeEnv::checkEmptyHardwareStateSyncTables()
{
    uint64_t tb_count = 0;
...
{
    checkEmptyHardwareStateSyncTables();
...
void crete_pre_cpu_tb_exec(void *qemuCpuState, TranslationBlock *tb)
{
    ...
    if(tb_pre_interested)
    {
        ...
        runtime_env->resetFlagHardwareStatePreInterest();
    }
        ...
        uint64_t crete_interrupted_pc)
{
	...
	if(static_flag_interested_tb)
	{
	    ...
	    if(reverse)
	    {
	        ...
	        {
	            runtime_env->addInitialHardwareState();
	        }
	        ...
	        if(rt_dump_tb_count == 1) {
	            ...
	            runtime_env->addEmptyHardwareStateSyncTable();
	        } else {
	            if(!static_flag_interested_tb_prev)
	            {
	                runtime_env->addHardwareStateSyncTable();
	            } else {
	                runtime_env->addEmptyHardwareStateSyncTable();
	            }
	        }
	        ...
	        // 8. vd trace
	        runtime_env->add_vd_tables(vd_ops_info.get());
	    }
    ...
    if(static_flag_interested_tb_prev && !static_flag_interested_tb) {
        runtime_env->setFlagHardwareStatePostInterest();
    }
...
void crete_add_vd_trace_marker_op_index(uint64_t vaddr, uint64_t physaddr,
        uint32_t size, uint64_t value, int is_write, uint64_t accessor)
{
    vd_ops_info.add_entry_func_info(vaddr, physaddr, size, value, is_write, accessor);
}
